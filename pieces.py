class Piece:
    images = ['white_king', 'white_queen', 'white_rook', 'white_bishop', 'white_nknight', 'white_pawn', 'black_king',
              'black_queen', 'black_rook', 'black_bishop', 'black_nknight', 'black_pawn']
...
class King(Piece):
        ...
        for x, y in {(x, y) for x in range(-2, 3) for y in range(-2, 3) if x != 0 and y != 0 and abs(x) != abs(y)}:
            try:
                ...
                if any(i < 0 for i in (coords[0], coords[1])):
                    continue
                if square and square.colour != self.colour and square.name == 'nknight' and coords != moved_to:
                    return True
            except IndexError:
                continue
        return False
    ...
    def can_castle(self, board):
        ...
        if board[self.back_rank][7] and board[self.back_rank][7].name == 'rook' and board[self.back_rank][7].castle_rights:
            ...
            #if all(not piece for piece in board[self.back_rank][5:7]) and all(
                    #not self.in_check(board, square) for square in squares):
                #moves1.append((6, self.back_rank))
            castle_r = castle_r + 'k'
        ...
        if board[self.back_rank][0] and board[self.back_rank][0].name == 'rook' and board[self.back_rank][0].castle_rights:
            ...
            #if all(not piece for piece in board[self.back_rank][1:4]) and all(
                    #not self.in_check(board, square) for square in squares):
                #moves1.append((2, self.back_rank))
            castle_r = castle_r + 'q'
        return str(castle_r)
...
class Knight(Piece):
    def __init__(self, colour):
        ...
        super().__init__(colour, 'nknight', img_adjust=(50, 52), unbounded=False)
